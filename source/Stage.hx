    ...
    public function new(daStage:String)
    {
        ...
        switch(daStage)
        {
			default:
					{
						curStage = 'safari';
						var bg:FlxSprite = new FlxSprite(-300,-550).loadGraphic(Paths.image('backbac', 'shared'));
						bg.scrollFactor.set(0.65, 0.65);
						toAdd.push(bg);
						var ground:FlxSprite = new FlxSprite(-450,-200).loadGraphic(Paths.image('floorshit', 'shared'));
						ground.scrollFactor.set(0.8, 0.8);
						toAdd.push(ground);
						var grass:FlxSprite = new FlxSprite(-275,500).loadGraphic(Paths.image('bg fg grass', 'shared'));
						grass.scrollFactor.set(0.97, 0.97);
						layInFront[2].push(grass);
					}
